...
def selects():
    escolha = input('Qual classe de personagem deseja buscar? (DPS | TANK | SUPORTE)\n')
    classe_maiuscula = escolha.upper()
    if classe_maiuscula not in ['DPS', 'TANK', 'SUPORTE']:
        print("Classe inválida. Por favor, escolha entre DPS, TANK ou SUPORTE.")
        return
    escolha = input('Deseja pesquisar um único personagem? (SIM | NÃO)\n')
    if escolha.upper() == 'SIM':
        nome_personagem = input('Digite o nome do personagem: ')
        
        # Proteção contra SQL Injection
        sql_nomes = f"SELECT descricao FROM Personagens WHERE nome = '{nome_personagem}'"
        cursor.execute(sql_nomes)  # Evita problemas de injeção SQL
        resultados = cursor.fetchall()
    # Verifica se encontrou resultados
        if resultados:
            for descricao in resultados:
                print(descricao[0])  # Exibe apenas o texto da descrição
        else:
            print(f"Personagem '{nome_personagem}' não encontrado!")

    elif escolha.upper() == 'NÃO':
            # Consulta de todos os personagens por classe
            sql_classe = f"SELECT nome, descricao FROM Personagens WHERE classe = '{classe_maiuscula}'"
            cursor.execute(sql_classe)
            resultados = cursor.fetchall()
            print(f"Personagens da classe {classe_maiuscula}\n")
            ...
            if not resultados: #Caso o banco não tenha personagens no banco de dados
                print(f"Nenhum personagem encontrado na classe '{classe_maiuscula}'.")
                return
